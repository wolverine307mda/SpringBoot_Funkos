...
class FunkoMapperTest {
    ...
    @Test
    void toFunkoWithInvalidCategoria() {
        FunkoDto funkoDto = new FunkoDto();
        funkoDto.setNombre("Spider Man");
        funkoDto.setPrecio(12.99);
        funkoDto.setStock(50);
        funkoDto.setCategoria("NonExistentCategory"); // Invalid category name
        ...
        assertAll(
                ...
                () -> assertEquals(funkoDto.getPrecio(), res.getPrecio()),
                () -> assertEquals(funkoDto.getStock(), res.getStock()),
                () -> assertNotEquals(funkoDto.getCategoria(), res.getCategoria().getNombre()) // This should handle invalid category gracefully
        );
    }

    @Test
    void toFunkoWithNoCategoria() {
        FunkoDto funkoDto = new FunkoDto();
        funkoDto.setNombre("Captain America");
        funkoDto.setPrecio(20.00);
        funkoDto.setStock(30);
        funkoDto.setCategoria(null); // No category provided

        var res = mapper.toFunko(funkoDto, null); // Null category passed to mapper

        assertAll(
                () -> assertEquals(funkoDto.getNombre(), res.getNombre()),
                () -> assertEquals(funkoDto.getPrecio(), res.getPrecio()),
                () -> assertEquals(funkoDto.getStock(), res.getStock()),
                () -> assertNull(res.getCategoria()) // Category should be null
        );
    }
}
